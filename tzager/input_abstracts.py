...
def abstract_augment_scopes(password, abstracts, data_for_scientific_analysis, complemetary_data, scopes, pmids=None):
    response = requests.post('http://tzagerlib1-env.eba-wjp8tqpj.eu-west-2.elasticbeanstalk.com/abstract_augment_scopes/' + password, json=json.dumps({'data_for_scientific_analysis': data_for_scientific_analysis, 'abstracts': abstracts, 'complemetary_data': complemetary_data, 'scopes': scopes, 'pmids': pmids}))
    if response.status_code == 200:
        data = dict(response.json())
    else:
        data = {'error': response.status_code}
